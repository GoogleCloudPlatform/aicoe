...
locals {
  templates = {
    ...
    "eval_ddos_settings.json" = "eval_ddos_settings.json.tpl",
  }
}
...
data "template_file" "generated" {
  ...
  vars = {
    ...
    composer_bucket             = local.composer_bucket
    containersec_incident_topic = data.google_pubsub_topic.containersec_incidents_pubsub_topic.name
    ...
    ddos_topic_name             = data.google_pubsub_topic.ddos_pubsub_topic.name
    ddos_topic_id               = data.google_pubsub_topic.ddos_pubsub_topic.id
    model_name                  = "%model_name%"
    endpoint_name               = local.endpoint_name
    endpoint_id                 = local.endpoint_id
    model_upload_location       = local.model_upload_location
    model_name_prefix           = local.model_name_prefix
  }
}
