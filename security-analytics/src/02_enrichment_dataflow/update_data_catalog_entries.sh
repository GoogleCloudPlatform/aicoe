...
gcloud data-catalog entries update --lookup-entry="pubsub.topic.\`${PROJECT_ID}\`.\`ddos-${RANDOM_SUFFIX}\`" --schema-from-file=./ddos/catalog_schema.yaml
gcloud data-catalog entries update --lookup-entry="pubsub.topic.\`${PROJECT_ID}\`.\`syn-${RANDOM_SUFFIX}\`" --schema-from-file=./syn/catalog_schema.yaml
