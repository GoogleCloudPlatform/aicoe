...
data "google_pubsub_topic" "containersec_incidents_pubsub_topic" {
  name = data.terraform_remote_state.s04_anomaly_detection.outputs.containersec_incidents_topic_name
}

data "google_pubsub_topic" "ddos_incidents_pubsub_topic" {
  name = data.terraform_remote_state.s04_anomaly_detection.outputs.ddos_incidents_topic_name
}
...
locals {
  ...
  composer_bucket                = data.terraform_remote_state.s03_feature_store.outputs.composer_bucket
  ...
  alerting_notification_channels = data.terraform_remote_state.s04_anomaly_detection.outputs.alerting_notification_channels
  endpoint_id                    = data.terraform_remote_state.s04_anomaly_detection.outputs.endpoint_id
  endpoint_name                  = data.terraform_remote_state.s04_anomaly_detection.outputs.endpoint_name
  model_upload_location          = data.terraform_remote_state.s04_anomaly_detection.outputs.model_upload_location
  model_name_prefix              = data.terraform_remote_state.s04_anomaly_detection.outputs.model_name_prefix
}
